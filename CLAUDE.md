...
This is a **Laravel 12 Livewire starter kit** — a boilerplate for new projects, not a domain application.

### Authentication

Authentication is fully handled by **Fortify** (headless). The configuration lives in:
- `config/fortify.php` — features enabled (registration, password reset, email verification, 2FA)
- `app/Providers/FortifyServiceProvider.php` — view bindings, rate limiting, action registration
- `app/Actions/Fortify/` — `CreateNewUser` and `ResetUserPassword` actions

Auth views are in `resources/views/pages/auth/`. Fortify handles routes automatically; do not define auth routes manually in `routes/web.php`.

### Settings Pages (Livewire)

Settings are Livewire full-page components routed in `routes/settings.php`:
...
The Livewire component classes live in `app/Livewire/` (currently only `Actions/Logout.php`). New settings components should follow this same pattern.

### Validation Traits

Reusable validation logic is extracted into traits in `app/Concerns/`:
...
- `layouts/app.blade.php` — authenticated layout with Flux sidebar + main content area
- `layouts/auth.blade.php` — unauthenticated centered layout
- `layouts/app/sidebar.blade.php` — sidebar navigation

### Database Defaults

All drivers default to **SQLite** (file: `database/database.sqlite`). Tests use SQLite `:memory:`. Production deployments should override via `.env`.
